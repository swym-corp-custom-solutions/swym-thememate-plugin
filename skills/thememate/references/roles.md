...
a standalone follow-up call). Use `support` for `swym_internal` + Support
team, `internal` for `swym_internal` + Success/ACQ, otherwise `agency` or
`merchant` directly.
...
record the answer via `--agency` (see [telemetry.md](telemetry.md)).
